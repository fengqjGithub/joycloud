...
public class TreeTest {
    static class Node {
        ...
        Map<String, Node> childMap = new LinkedHashMap<>();
        ...
        String printSelf(Node n, StringBuilder str) {//输出树的json结构
            ...
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    str.append("{code:").append(node.code).append(mark);
                    printSelf(node, str);
                    str.append("}");
                    if (i + 1 != list.size()) {
                        str.append(",");
    ...
    public static void main(String[] args) {
        ...
        Map<String, Node> rMap = new LinkedHashMap<>();// 临时组织数据map
    ...
    private static void turnToMap(Map<String, Node> rMap, Node n) {
        String key = null;
    ...
    private static void turnToList(Map<String, Node> rMap, Node rn) {
    ...
    private static void initTreeData(Node node) {
